/**
 * m4-gaussK-massweighted — intrinsic curvature of M⁴_L with the mass-weighted
 * kinetic-energy metric.
 *
 * Same layout as m4-gaussK: sphere colored by Gaussian curvature on the left,
 * live 4-rod chain on the right. The difference is the ambient inner product:
 * we use the M⊗I_2 metric from the "Realistic Sphere Linkages" post, with
 *   M = tridiag(a, b, a, b, ..., a),   a = 2 m_e/3 + m_v,   b = m_e/6,
 * and compute K via the Gauss equation
 *   K = (<II_pp, II_tt>_g - |II_pt|²_g) / (EG − F²),
 * with all inner products replaced by the M-weighted ones. Sliders for L, m_e,
 * m_v; default (m_e, m_v) = (0, 1) reproduces the unweighted post-4 demo.
 */
...
//
// Coupling matrix M for an n=4 chain (3 interior hinges) is the symmetric
// tridiagonal 3×3 with a on the diagonal and b on the adjacent off-diagonals.
// We pre-build the 6×6 ambient metric M⊗I_2 once per (m_e, m_v) and reuse it
// across all calls in a grid pass.
...
// <U, V>_g for U, V ∈ ℝ⁶ flattened as (p1x, p1y, p2x, p2y, p3x, p3y).
// Uses only the nonzero entries of M (3 diagonal a's, 4 off-diagonal b's).
function weightedDot6(g: AmbientMetric, U: number[], V: number[]): number {
  const { a, b } = g;
  // diagonal: a · (u·v at each hinge)
  const diag =
    a * (U[0]*V[0] + U[1]*V[1]) +
    a * (U[2]*V[2] + U[3]*V[3]) +
    a * (U[4]*V[4] + U[5]*V[5]);
  // off-diagonal: b · (cross terms between hinges 1↔2 and 2↔3, both directions)
  const cross =
...
// --- Gaussian curvature via numerical pullback ----------------------------
//
// Computes K(φ, t) = (<II_pp, II_tt>_g − |II_pt|²_g) / (EG − F²) directly,
// using a 9-point stencil on psi4Position. Mirrors the logic in
// firstAndSecondFundamentalForms from m4-bending, with the dot product
// replaced by `weightedDot6`.

function gaussianCurvature(phi: number, t: number, L: number, g: AmbientMetric): number {
  const eps = 1e-4;
  const e2 = eps * eps;
  ...
  // Subtract the g-tangent part of v to get its g-normal component.
  ...
  const II_pp = projectNormal(dPPp);
  const II_pt = projectNormal(dPTp);
  const II_tt = projectNormal(dTTp);

  const numK = weightedDot6(g, II_pp, II_tt) - weightedDot6(g, II_pt, II_pt);
  return numK / D;
}
...
function buildKGrid(L: number, m_e: number, m_v: number): {
  ...
  for (let j = 0; j < GRID_N; j++) {
    ...
    for (let i = 0; i < GRID_N; i++) {
      ...
      const k = gaussianCurvature(phi, tVal, L, g);
...
let m_e = 0.0;
let m_v = 1.0;
const initial = buildKGrid(L, m_e, m_v);
...
function rebuildField() {
  const { data, scalePos, scaleNeg, min, max } = buildKGrid(L, m_e, m_v);
...
sliderWrap.innerHTML = `
  <div class="slider-row"><span class="label">L</span>
    ...
    <span class="val" id="gk-L-val">${L.toFixed(2)}</span></div>
  <div class="slider-row"><span class="label">m_e</span>
    <input id="gk-me" type="range" class="thin-slider" min="0"    max="8"    step="0.01" value="${m_e}" />
    <span class="val" id="gk-me-val">${m_e.toFixed(2)}</span></div>
  <div class="slider-row"><span class="label">m_v</span>
    <input id="gk-mv" type="range" class="thin-slider" min="0"    max="4"    step="0.01" value="${m_v}" />
    <span class="val" id="gk-mv-val">${m_v.toFixed(2)}</span></div>
`;
document.body.appendChild(sliderWrap);

const lSlider  = sliderWrap.querySelector<HTMLInputElement>('#gk-L')!;
const meSlider = sliderWrap.querySelector<HTMLInputElement>('#gk-me')!;
const mvSlider = sliderWrap.querySelector<HTMLInputElement>('#gk-mv')!;
const lVal  = sliderWrap.querySelector<HTMLSpanElement>('#gk-L-val')!;
const meVal = sliderWrap.querySelector<HTMLSpanElement>('#gk-me-val')!;
const mvVal = sliderWrap.querySelector<HTMLSpanElement>('#gk-mv-val')!;

// Debounce the K-grid rebuild so dragging a slider doesn't rebuild
// 192² × O(stencil) every input event.
...
meSlider.addEventListener('input', () => {
  ...
  // Guard against the degenerate (0, 0) case → metric is identically zero.
