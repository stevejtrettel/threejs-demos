/**
 * Multi-Mesh Surface Studio
 *
 * Simple multi-mesh viewer that renders surfaces only.
 * Uses OBJSurface for lightweight surface rendering without vertices/edges.
 *
 * Features:
 * - Load multiple OBJ files via file picker
 * - Select meshes from the list with visual highlighting
 * - Per-mesh transform controls (position, rotation, scale)
 * - Per-group surface coloring
...
function addMesh(objString: string, name: string): MeshInstance {
    ...
    const mesh = OBJSurface.fromOBJ(objString);
...
const panel = new Panel('Multi-Mesh Surface Studio');
...
actionsFolder.add(new Button('Download Image', () => {
    ...
    app.renderManager.renderer.domElement.toBlob(blob => blob && saveAs(blob, `multi-surface-${ts}.png`));
}));
...
function rebuildSelectedMeshUI(): void {
    ...
    colorsFolder = new Folder('Surface Colors');

    for (const groupName of mesh.groups) {
        const displayName = groupName === 'default' ? 'Default' : `Group ${groupName}`;

        colorsFolder.add(new ColorInput(mesh.getGroupColor(groupName), {
            label: displayName,
            onChange: c => { mesh.setGroupColor(groupName, c); notifyPathTracerIfNeeded(); }
        }));
    }
...
app.start();
console.log('Multi-Mesh Surface Studio - Load OBJ files (surfaces only)');
