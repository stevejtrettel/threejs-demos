...
function getNextPosition(): THREE.Vector3 {
    ...
    const gridSize = 3; // spacing between meshes
...
const panel = new Panel('Multi-Mesh Studio');
...
actionsFolder.add(new Button('Download Image', () => {
    ...
    app.renderManager.renderer.domElement.toBlob(blob => blob && saveAs(blob, `multi-studio-${ts}.png`));
}));
...
let appearanceFolder: Folder | null = null;
...
function rebuildSelectedMeshUI(): void {
    ...
    appearanceFolder = null;
    colorsFolder = null;

    // Update mesh list highlight
    rebuildMeshListUI();
    ...
    appearanceFolder.add(new Toggle(settings.showVertices, {
        label: 'Show Vertices',
        onChange: v => { settings.showVertices = v; mesh.setVerticesVisible(v); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new Toggle(settings.showEdges, {
        label: 'Show Edges',
        onChange: v => { settings.showEdges = v; mesh.setEdgesVisible(v); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new Toggle(settings.showFaces, {
        label: 'Show Faces',
        onChange: v => { settings.showFaces = v; mesh.setFacesVisible(v); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new Slider(settings.sphereRadius, {
        label: 'Vertex Radius', min: 0.01, max: 0.15, step: 0.005,
        onChange: v => { settings.sphereRadius = v; mesh.setSphereRadius(v); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new Slider(settings.tubeRadius, {
        label: 'Edge Radius', min: 0.005, max: 0.08, step: 0.002,
        onChange: v => { settings.tubeRadius = v; mesh.setTubeRadius(v); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new ColorInput(settings.vertexColor, {
        label: 'Vertex Color',
        onChange: c => { settings.vertexColor = c; mesh.setVertexColor(c); notifyPathTracerIfNeeded(); }
    }));
    appearanceFolder.add(new ColorInput(settings.edgeColor, {
        label: 'Edge Color',
    ...
    for (const groupName of mesh.groups) {
        ...
        colorsFolder.add(new ColorInput(mesh.getFaceColor(groupName, 'front'), {
            label: `${displayName} Front`,
            onChange: c => { mesh.setFaceColor(groupName, 'front', c); notifyPathTracerIfNeeded(); }
        }));
        colorsFolder.add(new ColorInput(mesh.getFaceColor(groupName, 'back'), {
            label: `${displayName} Back`,
            onChange: c => { mesh.setFaceColor(groupName, 'back', c); notifyPathTracerIfNeeded(); }
        }));
    }
...
app.start();
console.log('Multi-Mesh Studio - Load OBJ files and click to select');
