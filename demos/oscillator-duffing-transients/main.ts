/**
 * oscillator-duffing-transients — initial conditions for the Duffing
 * oscillator still collapse onto a steady state, but the steady state
 * is no longer sinusoidal.
 *
 *   ẍ + 2γẋ + x + εx³ = cos(ω t).
 *
 * Same two-panel layout as oscillator-linear-transients: small (x, ẋ)
 ...
 * Sliding any of γ, ω, ε redraws every committed trajectory.
 */
...
//
// Integrates ẍ + 2γẋ + x + εx³ = cos(ω t) with RK4 over [0, T_MAX] at fixed
...
function integrateInto(positions: Float32Array, x0: number, v0: number) {
  ...
  for (let i = 1; i < N_SAMPLES; i++) {
    ...
    const k2x = v2;
    const k2v = -2 * gamma * v2 - x2 - eps * x2 * x2 * x2 + Math.cos(omega * (t + halfDt));
    // k3
    const x3 = x + halfDt * k2x;
    const v3 = v + halfDt * k2v;
    const k3x = v3;
    const k3v = -2 * gamma * v3 - x3 - eps * x3 * x3 * x3 + Math.cos(omega * (t + halfDt));
    // k4
    const x4 = x + DT * k3x;
    const v4 = v + DT * k3v;
    const k4x = v4;
...
const previewMat = new LineMaterial({
  color: SLATE_BLUE, linewidth: 3, worldUnits: false, depthTest: false,
});
