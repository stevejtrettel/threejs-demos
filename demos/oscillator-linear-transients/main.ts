 ...
 * gray with a small dot in phase space marking their IC. Sliding γ, ω, or A
 * redraws every trajectory.
 */
...
interface Params { gamma: number; omega: number; driveAmp: number; }

interface SteadyState { a: number; b: number; x0: number; v0: number; }

function steadyState({ gamma, omega, driveAmp }: Params): SteadyState {
  const D = (1 - omega * omega) ** 2 + 4 * gamma * gamma * omega * omega;
  const a = driveAmp * (1 - omega * omega) / D;
  const b = driveAmp * (2 * gamma * omega) / D;
...
// --- preview (Line2, maroon, thick) and active dot -----------------------
...
function fillPreview(p: Preview) {
  const f = solve({ gamma, omega, driveAmp }, p.x0, p.v0);
...
// Active dot in phase space follows the live preview's IC.
...
function fillCommitted(c: Committed) {
  const f = solve({ gamma, omega, driveAmp }, c.x0, c.v0);
  for (let i = 0; i < N_SAMPLES; i++) {
    const t = T_MIN + (T_MAX - T_MIN) * (i / (N_SAMPLES - 1));
    const [wx, wy] = timeToWorld(t, f(t));
...
equationLabel.innerHTML =
  ...
  '<span class="var">x</span> = <span class="var">A</span> cos(\u03c9<span class="var">t</span>)';
