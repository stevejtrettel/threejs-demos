/**
 * oscillator-quadratic-driven-motion — watch the damped driven quadratic
 * oscillator move against its cubic potential backdrop.
 *
 *   V(x) = 1/2 x^2 + (epsilon / 3) x^3
 *   x'' + 2 gamma x' + x + epsilon x^2 = A cos(omega t).
 *
 * The upper panel matches oscillator-quadratic-potential; the lower motion
 * rail shows the maroon ball moving horizontally from one fixed initial
 * condition under the same dynamics as oscillator-quadratic-transients.
 */
...
function acceleration(t: number, x: number, v: number): number {
  return -2 * gamma * v - x - eps * x * x + driveAmp * Math.cos(omega * t);
}
...
equationLabel.innerHTML =
  '<span class="title">driven motion</span>' +
