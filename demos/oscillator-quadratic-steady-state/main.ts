/**
 * oscillator-quadratic-steady-state — the steady-state response of the
 * driven damped quadratic oscillator
 *
 ...
 * ε biases and distorts the waveform; sliding γ changes the amplitude (most
 * strongly near ω = 1).
 *
...
function rk4Step(x: number, v: number, t: number, dt: number): [number, number] {
  ...
  const k2x = v2;
  const k2v = -2 * gamma * v2 - x2 - eps * x2 * x2 + driveAmp * Math.cos(omega * (t + half));
  const x3 = x + half * k2x, v3 = v + half * k2v;
  const k3x = v3;
  const k3v = -2 * gamma * v3 - x3 - eps * x3 * x3 + driveAmp * Math.cos(omega * (t + half));
...
// --- no steady-state overlay ---------------------------------------------

const unavailableShade = new THREE.Mesh(
  new THREE.PlaneGeometry(PLOT_W, PLOT_H),
...
unavailableShade.position.set(PLOT_CX, PLOT_CY, 0);
unavailableShade.renderOrder = 4;
unavailableShade.visible = false;
app.scene.add(unavailableShade);
...
function hasEscaped(x: number): boolean {
  return !Number.isFinite(x) || Math.abs(x) > ESCAPE_X;
}

function setSteadyStateAvailable(available: boolean) {
  line.visible = available;
  unavailableShade.visible = !available;
}
...
equationLabel.innerHTML =
  '<span class="title">steady state</span>' +
