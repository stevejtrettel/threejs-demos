...
//THIS ONE COMPUTES THE TIME DIRECTION CORRECTLY, NOT CONSTANT SPEED.
...
class BlackHoles {
    ...
    constructor(bhs = defaultbhs, scale = 1) {
        ...
        // Store original masses for scaling calculations
        ...
        for (let i = 0; i < bhs.length; i++) {
            ...
            let ehGeom = new CylinderGeometry(rad,rad,128,32,1,false);
    ...
    buildIntegrator() {
        ...
        let derive = function (state) {
            ...
            //acc is a 2D vector
            //we want the time direction to be controlled by forcing lightlike
            // this means U^-2 dt^2+U^2ds^2=0
            //simplifying gives a nice equation for tacc: -U |gamma dot| gammaDot*gradU

            let tAcc = u*Math.sqrt(vel2)*vel2D.dot(grad);

            let acc3D = new Vector3(tAcc,acc.x,acc.y);
    ...
    updatePos(index, pos) {
        ...
        // Format: (time, space_x, space_y) = (0, pos.x, pos.y)
        this.meshes.children[index].position.set(0, pos.x, pos.y);
