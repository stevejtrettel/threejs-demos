...
class NumericalCurve{
    constructor(pts, color=0xfffff, radius = 0.1,scale=1) {
        ...
        let ball = new SphereGeometry(2*this.radius*this.scale, 32,16);

        this.start = new Mesh( ball, this.material);
        let startPt = this.curve.getPoint(0).multiplyScalar(this.scale);
        this.start.position.set(startPt.x, startPt.y, startPt.z);

        this.end = new Mesh(ball, this.material);
        let endPt = this.curve.getPoint(1).multiplyScalar(this.scale);
    ...
    setCurve(pts){
        ...
        this.end.position.set(endPt.x, endPt.y, endPt.z).multiplyScalar(this.scale);
    }
