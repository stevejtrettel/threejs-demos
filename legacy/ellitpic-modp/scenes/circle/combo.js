...
let circMat = makeMaterial(colors.red);
let range = 1.;
let pts = [];
...
let curveMesh = new Mesh(curveGeom,circMat);

circ.add(curveMesh);


circ.position.set(3,0,-0.5);
scene.add(circ);
...
let linePath = new LineCurve3(start, end);
let lineGeom = new TubeGeometry(linePath,128,0.085);
let lineMesh = new Mesh(lineGeom,mat);
...
let fd = new Group();
fd.position.set(-3.4,0,0);

let fdStart = new Vector3(-1.,0,0);
let fdEnd = new Vector3(1.,0,0);
let fdPath = new LineCurve3(fdStart, fdEnd);
let fdGeom = new TubeGeometry(fdPath,128,0.085);
let fdMesh = new Mesh(fdGeom,mat);

fd.add(fdMesh);
scene.add(fd);


let fdPt = sphMesh.clone();
fdPt.position.set(-.6,0,0);
fd.add(fdPt);
...
let capMat = makeMaterial(colors.blue);
let capGeom = new CylinderGeometry(0.25,0.25,0.1);

let startCap = new Mesh(capGeom, capMat);
startCap.rotateZ(Math.PI/2);
let endCap = startCap.clone();

startCap.position.set(1.,0,0);
endCap.position.set(-1.,0,0);
