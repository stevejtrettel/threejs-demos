...
const scale = 0.5;
const dy = new Vector2(1/2,Math.sqrt(3)/2).multiplyScalar(scale);
...
for(let i=-layers;i<layers;i++){
    for(let j=-layers;j<layers;j++){
        ...
        pts.add(createBall(vert,0x33852c,0.03));

        let pt = vert.clone().add(new Vector2(-0.2,0.2));
        pts.add(createBall(pt,0x1e4d1a,0.04));
...
for( let i=-10; i<10; i++){
    edges.add(createHorizontal(i,colors.green,0.015));
    edges.add(createVertical(i,colors.green,0.015));
}
