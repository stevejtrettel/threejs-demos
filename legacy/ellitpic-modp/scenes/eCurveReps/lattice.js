...
const tau3 = new Vector2(1/2, Math.sqrt(3)/2);
const tau4 = new Vector2(0,1);
const tau7 = new Vector2(1/2, Math.sqrt(7)/2);
const tau8 = new Vector2(0,Math.sqrt(2));
const tau11 = new Vector2(1/2, Math.sqrt(11)/2);


let latticeColor = colors.purple;

const scale = 0.5;
const dy = tau11.clone().multiplyScalar(scale);
const dx = new Vector2(0.7,0).multiplyScalar(scale);
...
for(let i=-layers;i<layers;i++){
    for(let j=-layers;j<layers;j++){
        let pos = new Vector2();
        pos.add(dx.clone().multiplyScalar(i));
        pos.add(dy.clone().multiplyScalar(j));

        pts.add(createBall(pos,latticeColor,0.04));
