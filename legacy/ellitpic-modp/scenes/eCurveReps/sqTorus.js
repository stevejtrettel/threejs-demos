...
let surf = torus.getSurface(0xedafaf);
scene.add(surf);
...
for(let i=0; i<N+1; i++){
    ...
    let vert = torus.getEdgeAt(i/N,0xe38686,0.002,false);

    //for rectangle torus, the other side is "OPP EDGE"
    //for others, its a fiber
    let opp = torus.getOppEdgeAt(i/N,0xe38686,0.002,false);
    // let horiz = torus.getFiberAt(i/N,0x95b7cc,0.002,false);
...
lines.add(torus.getEdgeAt(0, colors.red,0.01,false));
lines.add(torus.getOppEdgeAt(0.65, colors.red,0.01,false));



let pt = latticeData.fromTauCoords([0.65,0]);
lines.add(torus.getPoint(pt,0xa82c2c,0.025));


//add a point in the fundamental domain
let mainPt = latticeData.fromTauCoords([0.5,0.4]);
scene.add(torus.getPoint(mainPt,0x781919,0.08));
