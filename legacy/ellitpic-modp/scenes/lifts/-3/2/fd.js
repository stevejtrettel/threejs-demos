...
for(let i=0; i<data.length;i++){
    points.add(fd.getDataPoint(data[i],0x318a29));
}




let height = 1.36602540378;
//draw the group orbit:

//the original group
let orbitColor = colors.yellow;
let orbitRad = 0.015;
let orbit = new Group();
scene.add(orbit);
let dir = new Vector2(-0.0384615,0.15544045).normalize().multiplyScalar(0.89);
orbit.add(fd.getLine([0,0], [dir.x,dir.y], orbitColor,orbitRad));
let s = Math.sqrt(2)/10;
orbit.add(fd.getLine([s,0], [s+dir.x, dir.y], orbitColor,orbitRad));
orbit.add(fd.getLine([2*s,0], [2*s+dir.x, dir.y], orbitColor,orbitRad));
orbit.add(fd.getLine([3*s,0], [3*s+dir.x, dir.y], orbitColor,orbitRad));
orbit.add(fd.getLine([4*s,0], [4*s+dir.x, dir.y], orbitColor,orbitRad));
orbit.add(fd.getLine([5*s,0], [5*s+dir.x, dir.y], orbitColor,orbitRad));

orbit.add(fd.getLine([6*s,0], [6*s+dir.x/2, dir.y/2], orbitColor,orbitRad));
orbit.add(fd.getLine([dir.x/2-s, dir.y/2], [dir.x-s, dir.y], orbitColor,orbitRad));
