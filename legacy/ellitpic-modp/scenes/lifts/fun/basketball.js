...
import normalTex from "/js/assets/ball_normal.png"
import colorTex from "/js/assets/ball_dark.png"
...
const colorTexture = new TextureLoader().load(colorTex)
const normalTexture = new TextureLoader().load(normalTex)
...
for(let i=0; i<data.length;i++){
    ...
    let sph = torus.getPoint(pt,0xb3b3b3,0.033,false);
    sph.material.normalMap = normalTexture;
    sph.material.map = colorTexture;
    sph.rotateX(Math.PI*Math.random());
    sph.rotateY(Math.PI*Math.random());
    sph.rotateZ(Math.PI*Math.random());
    points.add(sph);
}
//0.33
//0x2D728F
