...
let u1 = circle.getCircle(glassColor);
...
for(let i=0; i<8; i++){
    ...
    points.add(circle.getVertex(ang,colors.red,0.15));
}
...
ang = 1*2*Math.PI/8;
frob.add(circle.getRod(ang,3*ang,colors.green,0.025));
...
let fix1 = circle.getCurve(f1,colors.green,0.025,false);
frob.add(fix1);
...
let fix2 = circle.getCurve(f2,colors.green,0.025,false);
frob.add(fix2);
