...
import VertexViewS3 from "./VertexViewS3";
import FaceView from "./FaceView";
import SpringView from "./SpringView";
import SpringViewS3 from "./SpringViewS3";
...
export default class GeometryViewS3 extends Group {
    constructor(embedding, springSystems,color = {vertex:0x000000, edge:0x456abc, face:0xffffff }) {
        ...
        this.v = new VertexViewS3( verts, this.coords, color.vertex);
        this.f = new FaceView(faces, this.coords, color.face);
        this.s = [];
        for(let i=0;i<this.springSystems.length;i++){
            this.s.push(new SpringViewS3(this.springSystems[i],this.coords))
        }
