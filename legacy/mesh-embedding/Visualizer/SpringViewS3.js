...
function invStereo(p){
    return new Vector4(2*p.x,2*p.y,2*p.z,1-p.lengthSq()).divideScalar(1+p.lengthSq());
}


function sphDistance(p,q){
    let P = invStereo(p);
    let Q = invStereo(q);
    return Math.acos(P.dot(Q));
}
...
export default class SpringViewS3 extends InstancedMesh {
    ...
    sync() {
        ...
        for (let k = 0; k < this.springs.length; k++) {
            ...
            const j = s.j;
            const rest = s.rest;
           // const [i,j,rest] = this.springs[k];        // cached ints
            const a = 3 * i,   b = 3 * j;
            ...
            const rad = 1+mid.lengthSq();
            scale.set(rad,rad,len);
                              // (1,1,len)
            ...
            //---------SET THE COLOR--------------------
            //get the S3 distance
            const trueLen = sphDistance(start,end);
            const diff = trueLen - rest;
