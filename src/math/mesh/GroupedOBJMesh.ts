 ...
 * GroupedOBJMesh
 *
 * A reusable Three.js Group that renders OBJ geometry with:
 * - Front/back face meshes (path tracer compatible)
 * - Per-group face coloring
 * - Vertex spheres
 * - Edge tubes
 *
 * Designed for use with grouped OBJ files (using `g` directive).
 */
...
export interface GroupedOBJMeshOptions {
    // Visibility
    showFaces?: boolean;
...
const DEFAULT_OPTIONS: Required<Omit<GroupedOBJMeshOptions, 'faceColors'>> & { faceColors: Record<string, { front: string; back: string }> } = {
    showFaces: true,
    showVertices: true,
    showEdges: true,
    sphereRadius: 0.06,
...
export class GroupedOBJMesh extends THREE.Group {
    // Source data
    ...
    /**
     * Create a GroupedOBJMesh from parsed OBJ data
     */
    constructor(mesh: GroupedMesh, options: GroupedOBJMeshOptions = {}) {
        super();
    ...
    /**
     * Create a GroupedOBJMesh from an OBJ string
     */
    static fromOBJ(objString: string, options?: GroupedOBJMeshOptions): GroupedOBJMesh {
        const mesh = parseGroupedOBJ(objString);
        return new GroupedOBJMesh(mesh, options);
    }
